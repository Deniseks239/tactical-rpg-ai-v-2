...
static func get_story_intro_prompt(characters: Array) -> String:
	...
	return """
Придумай завязку сюжета и опиши первую локацию для начала приключения.

Персонажи игроков:
%s

Твой ответ должен состоять из двух частей, разделённых строкой "---":

Первая часть (до "---"): завязка сюжета — где находятся герои, почему они там, какая проблема перед ними.

Вторая часть (после "---"): описание первой локации — что это за место, какие враги/NPC рядом, есть ли выход.
