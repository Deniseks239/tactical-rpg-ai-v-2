...
func _start_llama_server():
	...
	check_request.request_completed.connect(func(result, code, headers, body):
		...
		if code == 200:
			...
			var output = []
			var pid = OS.create_process(exe_path, ["-m", model_path, "--n-gpu-layers", "99", "--ctx-size", "4096", "--port", "8080", "--reasoning", "off"], false)
			if pid > 0:
...
func _wait_for_server_ready():
	print("GameController: начинаю ожидание сервера...")
	while not llama_ready:
		await get_tree().create_timer(0.5).timeout
		print("GameController: проверяю сервер...")
		var http = HTTPRequest.new()
		add_child(http)
		http.request_completed.connect(func(_r, code, _h, _b):
			print("GameController: healthcheck ответ: ", code)
			if code == 200:
				print("GameController: сервер ответил 200, готов!")
				_on_llama_server_ready()
		)
